...
#
#
cart_1<- rpart(SalePrice~`Gr Liv Area`+`Overall Qual`+ Neighborhood, data=ames_raw_2008, method = 'anova')
summary(cart_1)
tree_prediction<-predict(cart_1,ames_raw_2009)
calc_loss(tree_prediction,ames_raw_2009$SalePrice)
```
...
#
#
sample_vec<-sample(1:1070,800)
OJ_training<-OJ[sample_vec,]
OJ_test<-OJ[-sample_vec,]
```
...
```{r,echo=TRUE}
#
cart_OJ<- rpart(Purchase~WeekofPurchase+STORE+PriceDiff,data=OJ_training,method='class')
summary(cart_OJ)
```
...
We can look at node 121, which classifies Purchase as MM. We can see from the splits above how to arrive at that terminal node, i.e week of purchase>247,store<3, 0.31>PriceDiff>=0.015. These tell us what range of values of each of these predictors classify Purchase as MM. We can also see that 29 of the observations at this node are MM while 20 are CH.
Your code:
```{r,echo=TRUE}
#
#
...
We can interpret this pretty easily by looking at what determines each split. The percent is the probability of being at that node. and the mm and ch indicate what the data at that node is classified as. 
...
```{r,echo=TRUE}
#
predicted_tree<-predict(cart_OJ,OJ_test,type='class')

#Confusion matrix:
confuse<-table(OJ_test$Purchase,predicted_tree)
confuse
...
```{r,echo=TRUE}
#
OJ_tree<-tree(Purchase~WeekofPurchase+STORE+PriceDiff,data=OJ_training)
cv.tree(OJ_tree)
```
